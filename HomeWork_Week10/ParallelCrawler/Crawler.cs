...
namespace ParallelCrawler
{
    ...
    {
        private ConcurrentDictionary<string, bool> allURLs; // 所爬取以及即将爬取的所有资源
        private ConcurrentQueue<string> waitingURLs; // 正在等待被爬取的队列
        ...
        public ParallelCrawler(string originURL, int crawlMaxCount = 10)
        {
            // 获取主网页
            this.baseURL = originURL.Split(new string[2] { "//", "/" }, StringSplitOptions.RemoveEmptyEntries)[1];

            this.allURLs = new ConcurrentDictionary<string, bool>();
            this.waitingURLs = new ConcurrentQueue<string>();

            // 主网页还尚未爬取
            this.allURLs.TryAdd(originURL, false);
        ...
        {
            while(crawlCount < this.timeLimit && waitingURLs.Count > 0)
            {
                // 取出一个待爬地址
                ...
                string nextURL;
                string htmlPage;
                if(this.waitingURLs.TryDequeue(out nextURL))
                {
                    Task<string> task = Task.Run(() => new WebClient().DownloadString(nextURL));
                    htmlPage = task.Result;
                    this.allURLs[nextURL] = true;
                    this.Parse(htmlPage, nextURL);    // 解析
                    // 一次爬取完成：取出网址——获取资源——解析资源，获取更多的链接网页进行爬取
                }

                // 如果没有将待爬取网站取出来，则等待下一个循环再取
            }
        }
            ...
            for(int i = 0; i < matchs.Count; i++)
            {
                ...
                if (Regex.IsMatch(nextAbsoluteURL, this.baseURL))
                {
                    ...
                    if (!this.allURLs.ContainsKey(nextAbsoluteURL))
                    {
                        ...
                        this.allURLs.TryAdd(nextAbsoluteURL, false);
                        this.waitingURLs.Enqueue(nextAbsoluteURL);
                        Console.WriteLine(nextAbsoluteURL);
                        crawlCount++;
                    }
                }
